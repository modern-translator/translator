...
// SINGLE ENGINE: the 5-model Flash cascade, used for everything (Flash-Lite
// models removed entirely). Highest tier first - cascades 3.8 -> 3.7 -> 3.6 -> 3.5
// -> 3 Flash, falling to the next lower model when the daily cap (or a real
// 429) is hit, then to the next API key, restarting the cascade at 3.8.
const GEMINI_MODELS = ['gemini-3.8-flash', 'gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash'];
...
const MAX_REQUESTS_PER_KEY_MODEL = 20;
...
const LAST_REQUEST_TIME_STORAGE_KEY = 'en2_translator_last_request_time_v1';
...
const App = () => {
  ...
  const [apiKeys, setApiKeys] = useState(() => {
    try {
      const raw = sessionStorage.getItem('en2_translator_api_keys') || localStorage.getItem('en2_translator_api_keys');
      if (raw) {
  ...
  const [rememberApiKey, setRememberApiKey] = useState(() => !!localStorage.getItem('en2_translator_api_keys'));
  ...
  const SESSION_STORAGE_KEY = 'en2_translator_session_v2';
  ...
  // Single engine: the 5-model Flash cascade (3.8 -> 3.7 -> 3.6 -> 3.5 -> 3),
  // 15s gap, 20 RPD cap per model+key, cascading through all active API keys.
  const callGemini = async (parts) => {
    ...
    const maxRequestsPerKeyModel = MAX_REQUESTS_PER_KEY_MODEL;
  ...
  const translatePageToEnglish = async (pageText, pageId, isPdf) => {
  ...
  // Verification & auto-correction pass. Runs through the same single Flash
  // cascade engine as everything else. Takes the source page + an existing
  // English draft, and asks the model to audit it - in a different cognitive
  // mode than producing it - against our catalogued failure modes, returning
  // either a sentinel meaning "no corrections needed" or a corrected HTML.
  const verifyAndCorrectTranslation = async (pageText, pageId, isPdf, englishHtmlDraft) => {
    ...
    const prompt = `
      ...
      ORIGINAL SOURCE PAGE TEXT (Arabic / Urdu - also see attached page image if provided):
      """
      ${pageText}
      """
  ...
  // Sequential (one-page-at-a-time) batch processing: each page gets exactly
  // ONE API call (translate only, via the 5-model Flash cascade) - no
  // automatic verification here, per instruction. Use the manual "Verify"
  // button per page if an audit/correction pass is wanted afterward.
  const startSequentialAnalysis = async () => {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      if (page.translationStatus !== 'done') {
        ...
        const englishHtmlResult = await translatePageToEnglish(page.content.rawText, page.id, page.isPdf);

        setParsedSections(prev => {
          const copy = [...prev];
          if (englishHtmlResult) {
            copy[i].englishHtml = englishHtmlResult;
            // Lock in the very first successful translation as the permanent
            // snapshot for this page, if one hasn't been saved yet. Later
            // manual edits, Verify runs, or re-translations never touch this -
            // "Reset to Original Translation" always returns to attempt #1.
            if (!copy[i].originalEnglishHtml) {
              copy[i].originalEnglishHtml = englishHtmlResult;
            }
  ...
  // "Verify" button: runs ONLY the verify/audit/correct pass against whatever
  // englishHtml currently exists for this page - 1 API call, via the same
  // 5-model Flash cascade. Works regardless of what produced the existing
  // draft (the batch translation or a manual edit).
  ...
  const handleSaveSettings = () => {
    ...
    sessionStorage.setItem('en2_translator_api_keys', JSON.stringify(keys));

    if (rememberApiKey) {
      localStorage.setItem('en2_translator_api_keys', JSON.stringify(keys));
    } else {
      // If the user opts out (or opts back out later), make sure no stale
      // keys are left behind in durable storage.
      localStorage.removeItem('en2_translator_api_keys');
    }
                    ...
                    >
                      ...
                      {s.translationStatus !== 'idle' && (
                        <div className="mt-2.5 pl-10 pr-1 flex items-center gap-1.5">
                          {/* Verify (audit & auto-correct current translation, 1 API request, icon-only) */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            title="Verify & auto-correct (Flash cascade: 3.8/3.7/3.6/3.5/3, 1 API request, 15s gap)"
