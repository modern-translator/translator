...
const LAST_REQUEST_TIME_STORAGE_KEY = 'bn2_translator_last_request_time_v1';
...
const App = () => {
  ...
  const [apiKeys, setApiKeys] = useState(() => {
    try {
      const raw = sessionStorage.getItem('bn2_translator_api_keys') || localStorage.getItem('bn2_translator_api_keys');
      if (raw) {
  ...
  const [rememberApiKey, setRememberApiKey] = useState(() => !!localStorage.getItem('bn2_translator_api_keys'));
  ...
  const SESSION_STORAGE_KEY = 'bn2_translator_session_v2';
  ...
  const translatePageToBangla = async (pageText, pageId, isPdf) => {
  ...
  // Bangla draft, and asks the model to audit it - in a different cognitive
  ...
  const startSequentialAnalysis = async () => {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      if (page.translationStatus !== 'done') {
        ...
        const banglaHtmlResult = await translatePageToBangla(page.content.rawText, page.id, page.isPdf);
        ...
        setParsedSections(prev => {
          ...
          if (banglaHtmlResult) {
            ...
            if (!copy[i].originalBanglaHtml) {
              copy[i].originalBanglaHtml = banglaHtmlResult;
            }
  ...
  // banglaHtml currently exists for this page - 1 API call, via the same
  ...
  const handleSaveSettings = () => {
    ...
    sessionStorage.setItem('bn2_translator_api_keys', JSON.stringify(keys));

    if (rememberApiKey) {
      localStorage.setItem('bn2_translator_api_keys', JSON.stringify(keys));
    } else {
      // If the user opts out (or opts back out later), make sure no stale
      // keys are left behind in durable storage.
      localStorage.removeItem('bn2_translator_api_keys');
    }
